          ...
          while (1) switch (_context8.prev = _context8.next) {
            case 0:
              sendRequests = function _sendRequests(list) {
                try {
                  var output = "Found requests: ```".concat(list.map(function (r) {
                    return "\u2022 ".concat(r.title ? "".concat(r.title, " - ") : '').concat(r.link ? "".concat(r.link, " - ") : '').concat(r.state, " - ").concat(r.user || 'Unknown User');
                  }).join('\n'), "```");
                  if (output.length >= 2000) {
                    var outputError = msg.author.id === '350059361789280277' ? 'Bitch :bingus:' : 'Result list was too big, be more specific or use the Requests section in the website';
